    ...
    namespace Main
    {
        public partial class Hospital_Change_Info_Form : Form
        {
            ...
            private void complete_btn_Click(object sender, EventArgs e)
            {
                ...
                String hospitalName = name_txt.Text;
                String hospitalDiff = hospital_division_combo.SelectedItem.ToString();
                String signficant = significant_txt.Text;
                int hospitalPhone = int.Parse(phone_num_txt.Text);
                int doctorNum = int.Parse(doctor_num_txt.Text);

                String first = first_location_combo.SelectedItem.ToString();
                String second = Second_location_combo.SelectedItem.ToString();
                String detailed = third_location_combo.Text;

                DB.update_hospital_info(PK , hospitalName, hospitalPhone, doctorNum, signficant, hospitalDiff);
                DB.update_location_info(PK, first, second, detailed);
        ...
        {
            DBconn2 DB = new DBconn2();
            hi = DB.Hospital_info(PK);
            location = DB.Location_info(PK);
            name_txt.Text = hi.hospitalName;
            hospital_division_combo.Text = hi.hospitalDivision;
            significant_txt.Text = hi.hospitalNote;
            phone_num_txt.Text = hi.hospitalPhone.ToString();
            doctor_num_txt.Text = hi.hospitalCount.ToString();

            first_location_combo.Text = location.first;
            Second_location_combo.Text = location.second;
            third_location_combo.Text = location.detailed;
